...
class RegionListViewTestCase(TestCase):
    ...
    def test_cannot_creates_duplicate_code_region(self):
        payload = {
            "code": "AL",
            "name": "Albania",
        }
        ...
        self.assertEqual(response.json().get("code")[0], "Region code already exists")

    def test_cannot_creates_without_code(self):
        payload = {
            "name": "Albania",
        }
        response = self.client.post(
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.json().get("code")[0], "Missing data for required field."
        )
